...
public partial class MainWindow : Window
{
    ...
    public IBrush getBGForTheme()
    {
        ...
        {
            BananaModManager.Theme.Light => Brush.Parse("#FFFFFF"),
            BananaModManager.Theme.Dark => Brush.Parse("#121212"),
            BananaModManager.Theme.DarkHighContrast => Brush.Parse("#000000"),
            BananaModManager.Theme.Sunrise => Brush.Parse("#F5E0C3"),
            BananaModManager.Theme.Frost => Brush.Parse("#D0E1F9"),
    ...
    public IBrush GetBorderBrush()
    {
        return GetTheme() switch
        {
            BananaModManager.Theme.Light => Brush.Parse("#E0E0E0"),
            BananaModManager.Theme.Dark => Brush.Parse("#2C2C2C"),
            BananaModManager.Theme.DarkHighContrast => Brush.Parse("#FFFFFF"),
            BananaModManager.Theme.Sunrise => Brush.Parse("#D7CCC8"),
            BananaModManager.Theme.Frost => Brush.Parse("#90A4AE"),
            _ => Brushes.Black
        };
    }
    ...
    public IBrush GetCardBGS()
    {
        return GetTheme() switch
        {
            BananaModManager.Theme.Light => Brush.Parse("#F9F9F9"),
            BananaModManager.Theme.Dark => Brush.Parse("#1F1F1F"),
            BananaModManager.Theme.DarkHighContrast => Brush.Parse("#000000"),
            BananaModManager.Theme.Sunrise => Brush.Parse("#FFE0B2"),
            BananaModManager.Theme.Frost => Brush.Parse("#BBDEFB"),
            _ => Brush.Parse("#FFFFFF")
        };
    }
    ...
    public IBrush getTextTheme()
    {
        return GetTheme() switch
        {
            BananaModManager.Theme.Light => Brush.Parse("#212121"),
            BananaModManager.Theme.Dark => Brush.Parse("#E0E0E0"),
            BananaModManager.Theme.DarkHighContrast => Brush.Parse("#FFFFFF"),
            BananaModManager.Theme.Sunrise => Brush.Parse("#6D4C41"),
            BananaModManager.Theme.Frost => Brush.Parse("#0D47A1"),
            _ => Brush.Parse("#FFFFFF")
        };
    }
    ...
    public IBrush GetSecondaryText()
    {
        return GetTheme() switch
        {
            BananaModManager.Theme.Light => Brush.Parse("#666666"),
            BananaModManager.Theme.Dark => Brush.Parse("#B0B0B0"),
            BananaModManager.Theme.DarkHighContrast => Brush.Parse("#FFFFFF"),
            BananaModManager.Theme.Sunrise => Brush.Parse("#6D4C41"),
            BananaModManager.Theme.Frost => Brush.Parse("#546E7A"),
            _ => Brush.Parse("#FFFFFF")
        };
    }
    void InitForRPC()
            ...
            client.SetPresence(new RichPresence
            {
                Buttons = new[]
                {
                    new DiscordRPC.Button
                    {
                        ...
                        Url = "https://github.com/Snoopy941/Banana-Mod-Manager/releases/latest"
                    },
                },
                Details = "Modding",
    ...
    public static async Task<(bool UpdateAvailable, Version LatestVersion)> IsUpdateAvailable(Version currentVersion)
    {
        try
        {
            ...
            string url = "https://raw.githubusercontent.com/Snoopy941/Banana-Mod-Manager/main/version";
    ...
    private static string GetConfigPath()
    {
        ...
        var folder = Path.Combine(appData, "BananaModManager");
    ...
    public static Theme GetTheme()
    {
        try
        {
            ...
            {
                var defaultConfig = new Config { Theme = BananaModManager.Theme.Dark };
                File.WriteAllText(configPath, JsonConvert.SerializeObject(defaultConfig, Formatting.Indented));
                return defaultConfig.Theme ?? BananaModManager.Theme.Dark;
            }

            var json = File.ReadAllText(configPath);
            var config = JsonConvert.DeserializeObject<Config>(json);

            return config?.Theme ?? BananaModManager.Theme.Dark;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error loading theme: {ex}");
            return BananaModManager.Theme.Dark;
        }
    }
    #endregion
    ...
    {
        // BepInEx is now shown as a core mod, so nothing is blacklisted by default.
    };
    ...
    public async Task LoadModsFromTheNewGitHubRepoAsync()
    {
        ...
        // The original list repo (The-Graze/MonkeModInfo) was deleted, so load a
        // bundled copy first (keeps core mods correct) and fall back to mirrors.
        string[] urls =
        {
            "https://raw.githubusercontent.com/swaggmastervr/MonkeModInfo/master/modinfo.json",
            "https://raw.githubusercontent.com/The-Graze/MonkeModInfo/master/modinfo.json"
        };
        ...
        try
        {
            ...
            try
            {
                var candidate = JsonConvert.DeserializeObject<List<Mod>>(LoadBundledModList());
            ...
            {
                lastError = ex;
                Console.WriteLine($"Loading bundled mod list failed: {ex.Message}");
            }
            ...
            if (mods == null)
            {
                foreach (var url in urls)
                {
                    try
                    {
                        ...
                        var candidate = JsonConvert.DeserializeObject<List<Mod>>(json);
                        if (candidate is { Count: > 0 })
                        {
                            mods = candidate;
                            break;
                    ...
                    {
                        lastError = ex;
                        Console.WriteLine($"Loading mods from {url} failed: {ex.Message}");
                    }
                }
            }
            ...
            if (mods == null)
            {
                throw new Exception("All mod list sources failed.", lastError);
            }
    ...
    private string LoadBundledModList()
    {
        using var stream = AssetLoader.Open(new Uri("avares://BananaModManager/Assets/modinfo.json"));
    ...
    private static IOrderedEnumerable<IGrouping<string, Mod>> OrderGroups(IEnumerable<IGrouping<string, Mod>> groups)
    {
        // Core (BepInEx, Utilla) always goes first, then the rest alphabetically.
        return groups
    ...
    private Border MakeModControl(Mod mod)
    {
        ...
        if (descriptionText != null)
        {
            descriptionText.Foreground = GetSecondaryText();
            secondaryTextBlocks.Add(descriptionText);
        }
        if (dependenciesText != null)
        {
    ...
    void MakeNotificationThing()
    {
        ...
        var bellButton = new Button
        {
            ...
            Content = new Image
            {
                Source = new Bitmap(AssetLoader.Open(new Uri("avares://BananaModManager/Assets/doorbell-google.png"))),
                Width = 28,
                Height = 28,
    ...
    private IBrush GetGroupColor(string group)
    {
        switch (CurrentTheme)
        {
            case BananaModManager.Theme.DarkHighContrast:
    ...
    private string GetModTargetDirectory(Mod mod)
    {
        // BepInEx is the loader itself; its zip extracts to the game root, not BepInEx/plugins.
        ...
        var installLocation = !string.IsNullOrEmpty(mod.InstallLocation)
            ? mod.InstallLocation
            : "BepInEx/plugins";
        return Path.Combine(gamePath, installLocation);
    }

    private bool IsModInstalled(Mod mod)
    {
        // BepInEx is installed by the manager at the game root.
        if (string.Equals(mod.Name, "BepInEx", StringComparison.OrdinalIgnoreCase))
            return Directory.Exists(Path.Combine(gamePath, "BepInEx"));
        ...
        try
        {
            ...
            var targetDirectory = GetModTargetDirectory(mod);
    ...
    private async Task NewVersionDialog(Version version)
    {
        ...
        {
            var url = "https://github.com/Snoopy941/Banana-Mod-Manager/releases/latest";
